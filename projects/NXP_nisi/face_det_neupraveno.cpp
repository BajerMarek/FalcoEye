 ...
 */
 ...
 void MODEL_ODPrintResult(const ODResult_t *p, int retCnt) {
     ...
     for (int i=0; i<retCnt; i++,p++) {
         PRINTF("%04d/1000, x1: %d, y1: %d, x2: %d, y2: %d, clr: %d, B_avg %d, R_avg %d, Pix_sum %d\r\n",
             p->score10, p->x1, p->y1, p->x2, p->y2,p->color,p->B_avg,p->R_avg,p->Pix_sum);
     }
 }
 ...
 void face_det()
 {
     ...
     while(1)
     {
         ...
         //! vlastni
         extern volatile uint32_t g_camera_buffer[];  // buffer kamery (RGB888)
         ...
         for(int i = 0; i < s_odRetCnt; i++) {
             ...
             s_odRets[i].B_avg = avgB;
             s_odRets[i].R_avg = avgR;
             s_odRets[i].Pix_sum = pixelCount;
